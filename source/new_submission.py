...
class NewSubmission:
    ...
    async def class_select_menu(self):
        ...
        embed.add_field(
            ...
            value=
            '''
            **Submitting will ERASE previous submissions for this contest, regardless of class.**
            ''',
            inline=False
        )
        embed.add_field(
        ...
        def check(react_got: discord.Reaction, user_got: discord.User):
            return user_got.id == self.user.id and (str(react_got) in self.player_reacts.values() or str(react_got) == "❌")
